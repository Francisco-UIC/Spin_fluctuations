...
%in a cell array consisting of 17 cells, where each cell represents the
...
Ef=17.644;          % (eV) Fermi energy determined from Au spectrum. Au was in 
                    % electrical contact with Bi2212 sample, so they have
...
%and subtract this to all data, since the background is almost
...
%Now that I have the background, I subtract it from all spectra:
...
%After subtraction, there is still some superlattice contribution, which we
%eliminate as follows.

for i=1:dims(3)
    ces=data70k(:,:,i);
    ces(ces<0.1*max(max(ces)))=0;    
    data70k(:,:,i)=ces; 
end
clear ces
...
fsi(118:end,:,:)=[];        %The cutoff value is determined "by eye".
dims=size(fsi);
fsi(isnan(fsi)==1)=[];
...
%  at 70K. It covers one quadrant of the BZ and energies considerably
%  beyond the extent of the antibonding band ([-640,640] meV)
