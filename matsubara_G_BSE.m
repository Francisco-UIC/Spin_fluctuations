...
% This is done in another script (BZ_quadrant.m); the data set must be
% ready before using this script.

% Some useful constants:

a = 3.86;                              % lattice constant (Angstrom)
...
% they have been interpolated to sammple one quadrant of the BZ evenly.

% bscesT variable should be loaded in workspace first !!!!! 
% (T is the termperature) 

A = eval(strcat('bsces',num2str(T)));       % spectral function as function of kx, ky, and w (3 dimension)
dims = size(A);
...
N = dims(1)*dims(2);   % total number of momentum points in Brillouin zone.
...
Np = length(wp);        % T-dependent # of Matsubara frequencies
...
% For the symmetrization scheme A(k,-w) = A(k,w), the following 
% simplification applies. Note that this gives a somewhat lower eigenvalue, 
% but still around unity. 

Gkw = 1i*imag(Gkw); 
% (Otherwise, comment out the above step and continue.)

% Pairing Kernel G(k,w)*G(k,-w)
P = flipdim(Gkw,3).*Gkw;
...
U = 0.67;                 % spin-fermion coupling; obtained from script "chi_w_wm", at each T.
for i = 1:n
...
iter = n+1;
ebin = ceil(Np/2);     % eV (this is the "wn=0" bin, which depends on Temperature) 
figure;
